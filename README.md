...
在“邮箱账号”勾选账号后点击“同步选中邮箱”，或点击“同步全部已启用邮箱”。任务后台执行，刷新页面查看最近检查时间与错误。暂停、需重置和已在队列中的账号会跳过；沿用原同步范围，每次最多收取 100 封。

在邮件详情底部确认后可“删除本地邮件”：清理本地正文及附件，从后台列表和公开分享中移除，并停止待发送/待重试任务。保留最小邮件 UID 去重标记与发送记录，因此重新轮询或重置同一 UIDVALIDITY 的同步范围不会重新导入该邮件。服务器原件不删除，已发送邮件不撤回。正在收发时请稍后重试删除。
